...
print("Loading GSWesteros_Positioning.lua");
if(not silent) then
...
if (((g_iW==45) and (g_iH==80)) or ((g_iW==96) and (g_iH==126))) then
  ...
  function Initialize()
    ...
    for loop, pPlayer in ipairs(aPlayers) do
      ...
      if(pPlayer ~= nil) then
        if(GameInfo.Leaders["LEADER_NIGHTKING"].Hash == config:GetLeaderTypeID()) then --The North
          if(not silent) then
            print("Found Night King, PlayerID: "..iPlayer);
          end
          iNightKingPLayer=iPlayer;
        elseif(GameInfo.Leaders["LEADER_EDDARD"].Hash == config:GetLeaderTypeID()) then --The North
          if(not silent) then
...
print("GSWesteros_Positioning.lua loaded");